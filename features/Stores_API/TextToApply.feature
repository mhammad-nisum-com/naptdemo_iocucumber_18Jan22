...
Feature: Create APIs to initiate POS based enrolment in Orchestration service
  ...
  Scenario: Apply For Card Through Text
    Given that param "base_url.url" is set to value "http://strwebappsqa2.wsgc.com/CreditApplication"
    ...
    When I make a "POST" REST Call with URL "/api/CAS/v1/initiateCASProcess" and Body from Dictionary Key "initiateCASProcess"
    Then I verify that the response code is "200" for the response with Dictionary Key "initiateCASProcess" and get sessionID
    # ******postDLDetails******
    ...
    When I make a "POST" REST Call with URL "/api/CAS/v1/sessionID/postDLDetails" and Body from Dictionary Key "PostDLDetails" and replace the sessionID
    Then I verify that the response code is "200" for the response with Dictionary Key "PostDLDetails"
    # ******getQuestions******
    When I make a "GET" REST Call with URI "/api/CAS/v1/sessionID/getQuestions" and store the response of sessionID with Dictionary Key "GetQuestions"
    ...
    When I make a "POST" REST Call with URL "/api/CAS/v1/sessionID/postPIIData" and Body from Dictionary Key "PostPIIData" and replace the sessionID
    Then I verify that the response code is "200" for the response with Dictionary Key "PostPIIData"
   # ******getApplicationStatus******
    When I make a "GET" REST Call with URI "/api/CAS/v1/sessionID/getApplicationStatus" and store the response of sessionID with Dictionary Key "GetApplicationStatus"
