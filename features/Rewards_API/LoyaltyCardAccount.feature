...
Feature: Card feature
  ...
  @get @GetTokenWithtokenTypeandtokenId
  Scenario: Get Token with tokenType and tokenId
    Given that param "headers.Content-Type" is set to value with bearer token  "application/json"
    And that param "headers.Authorization" is set to value "access_token"
    When I make a "TOKEN_TYPE" Call with tokenType and tokenId from Dictionary Key "CREATE_TOKEN" and "/loyalty/v1/loyaltyCards/token/"
    Then I verify the response with status code "200" for Dictionary Key "CREATE_TOKEN"